  ...
  <ApolloQuery class="container" :query="query">
    <template slot-scope="{ result: { loading, error, data } }">
      <section class="main">
        ...
        <section v-if="data">
          ...
          <section v-if="data.lookup.artist.releaseGroups">
            <h2>Albums</h2>
            <ul class="releases">
              <li class="release" v-bind:key="release.node.mbid" v-for="release in data.lookup.artist.releaseGroups.edges" >
                <router-link class="release__title" :to="{ name: 'releaseDetails', params: { releaseId: release.node.mbid }}">{{ release.node.title }} ({{ release.node.firstReleaseDate }})</router-link>
      ...
      <aside v-if="data" class="side">
        <img class="black-n-white" :src="data.lookup.artist.theAudioDB.thumbnail" />
      </aside>
    </template>
  </ApolloQuery>
</template>
